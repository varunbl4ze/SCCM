/* ==========================================================================
   ...
   Talks to: POST /api/complaints/, then POST /api/complaints/<id>/image
   if a photo was attached. Precise geolocation is wired as far as the
...
/* ---- Image upload (preview here; actual upload happens on submit) -------
   The file is kept in the <input type="file"> and uploaded via
   API.images.upload() inside initComplaintForm()'s submit handler, once
   the complaint has been created and we have an id to attach it to. */
...
function initComplaintForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      // If the citizen attached a photo, upload it now that we have a
      // complaint id to attach it to (POST /api/complaints/<id>/image).
      const imageInput = document.getElementById('complaintImage');
      const selectedFile = imageInput?.files?.[0];
      if (selectedFile) {
        try {
          await API.images.upload(complaint.id, selectedFile);
        } catch (imgErr) {
          // The complaint itself was filed successfully — don't block on a
          // photo failure, just let the citizen know they can retry later
          // from the complaint details page.
          showToast(`Complaint filed, but the photo failed to upload: ${imgErr.message}`, 'error');
        }
      }

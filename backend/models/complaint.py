...
class Complaint(db.Model):
    ...
    # Admin-panel assignment fields — set via /api/admin/complaints/<id>/assign.
    # Left nullable so existing complaints created before this feature
    # existed remain valid (unassigned).
    department_id = db.Column(db.Integer, db.ForeignKey("departments.id"), nullable=True)
    assigned_admin_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=True)

    # Explicit foreign_keys needed since this is the *second* FK from
    # Complaint to User (the first is user_id / the citizen who filed it).
